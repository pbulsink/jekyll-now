### Patch for full.game.database and for player.summary for A.C. Thomas's nhlscrapr package version 1.8 ( act@acthomas.ca ) by Jack Davis 2016-04-18
### ( jackd@sfu.ca )
...
full.game.database <- function(extra.seasons = 0) {
    ...
    playoff.series <- c("11", "12", "13", "14", "15", "16", "17", "18", "21", "22", "23", "24", "31", "32", "41")
    gnum <- paste0("0", c(t(outer(playoff.series, 1:7, paste0))))
    for (ss in 1:length(seasons)) {
        gn1 <- as.character(1:games[ss])
        while (any(nchar(gn1) < 4)) gn1[nchar(gn1) < 4] <- paste0("0", gn1[nchar(gn1) < 4])
        df1 <- data.frame(season = seasons[ss], session = c(rep("Regular", games[ss]), rep("Playoffs", length(gnum))), gamenumber = c(gn1, gnum), gcode = "",
            status = 1, valid = c(!(1:games[ss] %in% bad.game.list[[ss]]), rep(TRUE, length(gnum))), awayteam = "", hometeam = "", awayscore = "",
            homescore = "", date = "", game.start = "", game.end = "", periods = 0, stringsAsFactors = FALSE)
    ...
    bad.playoff <- matrix(c("20032004", "30134", "20052006", "30233"), nrow = 2)
    for (kk in 1:dim(bad.playoff)[2]) game.roster$status[game.roster$season == bad.playoff[1, kk] & game.roster$gcode == bad.playoff[2, kk]] <- 0
...
player.summary <- function(grand.data, roster.unique) {
    ...
    for (ee in events) {
        ...
        if (dim(little.data)[1] > 0) {
            # patched this line, cc is an index now - JD
            for (cc in which(names(grand.data) %in% c("ev.player.1", "ev.player.2", "ev.player.3"))) {
            ...
            # patched this line, cc is an index now - JD
            for (cc in which(names(grand.data) %in% c("a1", "a2", "a3", "a4", "a5", "a6", "away.G"))) {
            ...
            ## patched this line, cc is an index now - JD
            for (cc in which(names(grand.data) %in% c("h1", "h2", "h3", "h4", "h5", "h6", "home.G"))) {
                evs <- table(little.data[little.data$ev.team == little.data$hometeam, cc])
                rws <- match(as.numeric(names(evs)), involved.players)
                evs <- evs[!is.na(rws)]  ### added this line, this removes player that were never involved in that situation (NAs in match() )
                rws <- rws[!is.na(rws)]  ### added this line, this removes player that were never involved in that situation (NAs in match() )
...
aggregate.roster.by.name <- function(roster) {
    ...
    roster_name = ddply(roster, .(firstlast), summarize, pos = pos[1], last = last[1], first = first[1], numfirstlast = numfirstlast[1], firstlast = firstlast[1],
        index = index[1], player.id = player.id[1], pC = sum(pC), pL = sum(pL), pR = sum(pR), pD = sum(pD), pG = sum(pG))

    roster_name = roster_name[order(roster_name$player.id), ]
    return(roster_name)
}

compile.all.games <- function(rdata.folder = "nhlr-data", output.folder = "source-data", new.game.table = NULL, seasons = NULL, verbose = FALSE, override.days.back = NULL,
    ...
    if (file.exists(paste0(output.folder, "/nhlscrapr-core.RData"))) {
        ...
        for (kk in blanks[blanks > 1 & blanks < nrow(games)]) if (!is.na(games$date[kk - 1]) && !is.na(games$date[kk + 1]) && games$date[kk - 1] ==
            games$date[kk + 1])
    ...
    for (this.season in unique(games$season)) {
        ...
        for (kk in 1:nrow(sub.games)[1]) {
            if (kk%%500 == 0)
                message(paste0("Event assembly: ", this.season, " game", kk))
            if (verbose)
                message("Trying game ", sub.games$season[kk], sub.games$gcode[kk], " ", sub.games$date[kk], " because it's ", today.now)
            if (grepl("[0-9]{4}\\-[0-9]{2}\\-[0-9]{2}", sub.games$date[kk]) | !date.check) {
                if (sub.games$date[kk] > today.now) {
                  if (verbose)
                    message("Skipping Game ", sub.games$season[kk], sub.games$gcode[kk], " ", sub.games$date[kk], " because it's ", today.now)
                  next
                }
            } else next
            tryme <- try({
                game.info <- retrieve.game(sub.games$season[kk], sub.games$gcode[kk], rdata.folder, force = FALSE)
                doit <- FALSE
                if (is.null(game.info))
                  doit <- TRUE else if (game.info$status %in% 1:2)
                  doit <- TRUE
                if (doit)
                  game.info <- process.single.game(sub.games$season[kk], sub.games$gcode[kk], rdata.folder = rdata.folder, override.download = TRUE,
                    ...)
            ...
            if (class(tryme) == "try-error")
                cons.failures <- cons.failures + 1 else cons.failures <- 0
            if (cons.failures >= 20) {
        ...
        if (!is.null(secondary.data)) {
            ...
            grand.data <- rbind(grand.data[!(grand.data$gcode %in% unique(secondary.data$gcode)), ], secondary.data)
...
### From this point, copied functions that were masked from nhlscrapr after new version of full.game.database and compile.all.games

construct.rosters.from.list <- function(roster.collection, roster.master = NULL) {
    # roster.collection=new.roster; roster.master=NULL

    blanky <- data.frame(pos = "", last = "", first = "", numfirstlast = "", firstlast = "", index = 1, player.id = 1, pC = 0, pL = 0, pR = 0, pD = 0,
        pG = 0, stringsAsFactors = FALSE)
    if (is.null(roster.master))
        roster.master <- blanky

    for (kk in 1:length(roster.collection)) if (!is.null(roster.collection[[kk]]))
        if (nrow(roster.collection[[kk]]) > 0) {
            # ) { #

            if (kk%%500 == 0)
                message(paste("Roster merger: game", kk, "of", length(roster.collection)))

            this.roster <- fix.names.manually(roster.collection[[kk]][, c("number", "pos", "last", "first", "numfirstlast")])

            match1 <- match(this.roster$numfirstlast, roster.master$numfirstlast)
            if (any(is.na(match1))) {
                rows <- which(is.na(match1))
                newrecs <- data.frame(pos = this.roster$pos[rows], last = this.roster$last[rows], first = this.roster$first[rows], numfirstlast = this.roster$numfirstlast[rows],
                  firstlast = "", index = nrow(roster.master) + 1:length(rows), player.id = NA, pC = 0, pL = 0, pR = 0, pD = 0, pG = 0, stringsAsFactors = FALSE)

                newrecs$firstlast <- paste(newrecs$first, newrecs$last)

                m2 <- match(newrecs$firstlast, roster.master$firstlast)
                if (any(!is.na(m2)))
                  newrecs$player.id[!is.na(m2)] <- roster.master$player.id[m2[!is.na(m2)]]
                if (any(is.na(m2)))
                  newrecs$player.id[is.na(m2)] <- max(roster.master$player.id) + 1:sum(is.na(m2))

                roster.master <- rbind(roster.master, newrecs)

                # zeroes <- rep(0, length(rows)) positions <- rbind(positions, data.frame(pC=zeroes, pL=zeroes, pR=zeroes, pD=zeroes, pG=zeroes))
            }

            r1.match <- match(this.roster$numfirstlast, roster.master$numfirstlast)
            roster.master$pC[r1.match[this.roster$pos == "C"]] <- roster.master$pC[r1.match[this.roster$pos == "C"]] + 1
            roster.master$pL[r1.match[this.roster$pos == "L"]] <- roster.master$pL[r1.match[this.roster$pos == "L"]] + 1
            roster.master$pR[r1.match[this.roster$pos == "R"]] <- roster.master$pR[r1.match[this.roster$pos == "R"]] + 1
            roster.master$pD[r1.match[this.roster$pos == "D"]] <- roster.master$pD[r1.match[this.roster$pos == "D"]] + 1
            roster.master$pG[r1.match[this.roster$pos == "G"]] <- roster.master$pG[r1.match[this.roster$pos == "G"]] + 1
...
fix.names.manually <- function(master.list) {
    ...
    master.list <- master.list[order(master.list$last, master.list$first), ]
...
.simpleCap <- function(x, ch = " ", ch2 = ch) {
    s <- strsplit(x, ch)[[1]]
    paste(toupper(substring(s, 1, 1)), tolower(substring(s, 2)), sep = "", collapse = ch)
}
...
manual.patches <- function(roster.unique) {
    ...
    substr(roster.unique$last[grep("^Mac", roster.unique$last)], 4, 4) <- toupper(substr(roster.unique$last[grep("^Mac", roster.unique$last)], 4, 4))
    substr(roster.unique$last[grep("^Mc", roster.unique$last)], 3, 3) <- toupper(substr(roster.unique$last[grep("^Mc", roster.unique$last)], 3, 3))
    ...
    if (length(frame.list) > 1)
        repeat {
            if (length(frame.list) == 1)
                break
            hold.list <- list()
            for (kk in 1:floor(length(frame.list)/2)) hold.list[[kk]] <- tryCatch(rbind(frame.list[[2 * kk - 1]], frame.list[[2 * kk]]), warning = function(war) message(paste(kk,
                war)), error = function(err) message(paste(kk, err)), finally = {
            })
            if (length(frame.list)%%2 == 1)
                if (length(hold.list) > 0)
                  hold.list[[length(hold.list)]] <- rbind(hold.list[[length(hold.list)]], frame.list[[2 * kk + 1]]) else hold.list <- frame.list[2 * kk + 1]
...
augment.game <- function(game.info, player.list) {
    ...
    if (length(playbyplay) == 0)
        stop("Play-by-play table does not exist.")
    if (length(player.list) == 0)
        stop("Player roster does not exist.")
    ...
    for (cc in c(paste0("a", 1:6), paste0("h", 1:6), "away.G", "home.G", "ev.player.1", "ev.player.2", "ev.player.3")) {
        ...
        if (is.null(replacement))
            replacement <- rep(NA, dim(playbyplay)[1])
...
pick.section <- function(xy.points) {
    ...
    picker <- function(row) if (sum(row) > 0)
        min(which(row)) else 0
