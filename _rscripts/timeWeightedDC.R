...
DCmodelData <- function(df) {
    ...
    return(list(homeTeamDMa = hm.a, homeTeamDMd = hm.d, awayTeamDMa = am.a, awayTeamDMd = am.d, homeGoals = df$HG, awayGoals = df$AG, dates = df$Date, 
        teams = team.names))
}
...
DCoptimFn <- function(params, DCm, xi = 0, currentDate = Sys.Date()) {
    ...
    return(DClogLik(y1 = DCm$homeGoals, y2 = DCm$awayGoals, lambda, mu, rho.p, w) * -1)
}
