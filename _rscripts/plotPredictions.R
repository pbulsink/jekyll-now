plotPredictions <- function(prediction_results, title = "Prediction of results", n_sims = 1) {
    ...
    p <- ggplot(df_results_m, aes(x = value)) + geom_density(alpha = 0.3, adjust = 1.5) + facet_grid(Team ~ ., scales = "free") + theme(strip.text.y = element_text(angle = 0), 
        strip.background = element_blank()) + scale_y_continuous(breaks = NULL) + xlab("End-Of-Season Position") + ylab("Probability") + ggtitle(paste0(title, 
