...
getPlayerList <- function(sleep = 30) {
    ...
    player_list <- data.frame(Complete = character(), BlnNHL = character(), URL = character(),
        Name = character(), Active = character())
...
getPlayerStats <- function(player_list, sleep = 30) {
    ...
    for (player in c(1:nrow(plist))) {
        ...
        player_meta_tables <- rbind.fill(player_meta_tables, data.frame(Name = pname,
...
scrapeByAlphabet <- function(player_list, letters_to_scrape = letters, letter_sleep = 120,
    combine = TRUE, directory = "./_data/players/", ...) {
