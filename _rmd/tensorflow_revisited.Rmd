...
run_epoch<-function(session, model, eval_op=NULL, verbose=FALSE){
  ...
  for (step in c(1:model$input$epoch_size)){
    ...
    #this is awkward in R because of poor dictionary support from reticulate
    ncell<-length(model$initial_state)
    if(ncell == 1){
        feed_dict<-dict('1'=tuple(c=state[[1]]$c, 'h'=state[[1]]$h))
    } else if (ncell == 2){
        feed_dict<-dict('1'=tuple(c=state[[1]]$c, 'h'=state[[1]]$h),
                        '2'=tuple(c=state[[2]]$c, 'h'=state[[2]]$h))
    } else if (ncell == 2){
        feed_dict<-dict('1'=tuple(c=state[[1]]$c, 'h'=state[[1]]$h),
                        '2'=tuple(c=state[[2]]$c, 'h'=state[[2]]$h),
                        '3'=tuple(c=state[[3]]$c, 'h'=state[[3]]$h))
    } else if (ncell == 2){
        feed_dict<-dict('1'=tuple(c=state[[1]]$c, 'h'=state[[1]]$h),
                        '2'=tuple(c=state[[2]]$c, 'h'=state[[2]]$h),
                        '3'=tuple(c=state[[3]]$c, 'h'=state[[3]]$h),
                        '4'=tuple(c=state[[4]]$c, 'h'=state[[4]]$h))
    }
