...
Model<-R6Class('Model', 
    public = list(
        ...
        initialize = function(is_training, params, inputs){
            ...
            self$output <- tf$reshape(tensor = o, shape = shape(-1L, self$hidden_size))  
...
run_epoch<-function(session, model, eval_op=NULL, verbose=FALSE){
  ...
  for (step in c(1:model$input$epoch_size)){
    ...
    if(ncell == 1){
        c1<-state[[1]]$c
        h1<-state[[1]]$h
        feed_dict<-dict(c1 = state[[1]]$c, h1 = state[[1]]$h)
    } else if (ncell == 2){
        c1<-state[[1]]$c
        h1<-state[[1]]$h
        c2<-state[[2]]$c
        h2<-state[[2]]$h
        feed_dict<-dict(c1 = state[[1]]$c, h1 = state[[1]]$h,
                        c2 = state[[2]]$c, h2 = state[[2]]$h)
    }  # else if (ncell == 3){
    #     feed_dict<-dict((state[[1]]$c) = state[[1]]$c, (state[[1]]$h) = state[[1]]$h,
    #                     (state[[2]]$c) = state[[2]]$c, (state[[2]]$h) = state[[2]]$h,
    #                     (state[[3]]$c) = state[[3]]$c, (state[[3]]$h) = state[[3]]$h)
    # } else if (ncell == 4){
    #     feed_dict<-dict((state[[1]]$c) = state[[1]]$c, (state[[1]]$h) = state[[1]]$h,
    #                     (state[[2]]$c) = state[[2]]$c, (state[[2]]$h) = state[[2]]$h,
    #                     (state[[3]]$c) = state[[3]]$c, (state[[3]]$h) = state[[3]]$h,
    #                     (state[[4]]$c) = state[[4]]$c, (state[[4]]$h) = state[[4]]$h)
    # }
    
    message('feed_dict')
    message(feed_dict)
    stop()
    # for (i in 1:length(model$initial_state)){
    #   feed_dict[[i]]<-tuple('c'=state[[i]]$c, 'h'=state[[i]]$h)
    #   message('feed_dict[[i]]')
    #   message(feed_dict[[i]])
    # }
    ...
    message('costs')
    cost <- session$run(model$cost, feed_dict=dict(feed_dict))
    message('state')
    state <- session$run(model$final_state, feed_dict=dict(feed_dict))
