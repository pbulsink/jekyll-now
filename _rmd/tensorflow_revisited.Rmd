...
Model<-R6Class('Model', 
    public = list(
        ...
        assign_lr=function(session, lr_value){
            self$new_lr<-lr_value
            new_lr<-self$new_lr
            message(self$new_lr)
            session$run(self$lr_update, feed_dict=dict(new_lr = self$lr_value))
        },
...
```{r main, eval=FALSE}
main<-function(datapath="./_data/", verbose=TRUE, savepath="./data/tfmodels/", params_type='small'){ 
    require(tensorflow)
    require(R6)
