...
getAndPrepAllData<-function(year_list=c(2006, 2007, 2008, 2009,2010,2011,2012,2013,2014,2015,2016)){
    ...
    for (year in 1:length(year_list)){
        df<-rbind(df, read.csv(paste('./_data/',year_list[year]-1,year_list[year],".csv", sep=''))[c("Date", "Visitor", "G", "Home", "G.1", "X")])
    }
...
```{r goals_vs_poisson_plot, echo=FALSE}
library(ggplot2)
...
```{r all_res, warning=FALSE, echo=FALSE}
t<-proc.time()
nhl.all.res<-doDCPrediction(nhl_all)
t_all_calc<-proc.time()-t
```

```{r one_res, warning=FALSE, echo=FALSE}
t<-proc.time()
...
```{r attack_defence_corr_plot, echo=FALSE}
library(ggplot2)
library(ggrepel)
...
for (t in 1:length(as.character(unique(nhl2015$AwayTeam)))){
     ...
     team_params[team_params$Team == team,]$Defence=nhl.one.res$par[paste("Defence", team, sep=".")]
}

ggplot(team_params, aes(x=Attack, y=Defence, color=Team, label=Team)) +
    ggtitle("Attack vs Defence Parameters") +
    xlab("Attack") + 
    ylab("Defence") +
    geom_point() +
    geom_text_repel(force=2, max.iter=5000) +
...
```{r xi_all_res, warning=FALSE, echo=FALSE}
t<-proc.time()
nhl_all_res<-doDCPrediction_w(nhl_all, xi=0.005)
t_all_calc<-proc.time()-t
```

```{r xi_one_res, warning=FALSE, echo=FALSE}
t<-proc.time()
...
We'll plot the Attack and Defence parameters for each team to se if there's any correllation.
```{r xi_attack_defence_corr_plot, echo=FALSE}
library(ggplot2)
library(ggrepel)
team_params<-data.frame("Team"=unique(nhl2015$AwayTeam), "Attack"=0, "Defence"=0)
for (t in 1:length(as.character(unique(nhl2015$AwayTeam)))){
