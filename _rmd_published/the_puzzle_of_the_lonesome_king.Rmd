...
removerLoop<-function(s){
    subjects_remaining<-length(s)
    while (subjects_remaining > 1){
        s<-s[-sample(1:subjects_remaining, replace=TRUE)]
        subjects_remaining<-length(s)
    }
...
```{r simulate_removal, warnings=FALSE}
subjects_in_competition <- c(1:n_subjects)
nreps<-1000
cl <- makeCluster(detectCores() - 1)
registerDoParallel(cl)
...
This gives us a winning subject proportion of  `r prop_crowned`.
...
For the generic set, we can see it hovers around the 0.68 that we achieved for our 56,000 population kingdom. There are generic formulaic terms that would provide this, but I haven't been able to parse them out yet. 
