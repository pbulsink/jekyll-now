...
doDCPrediction<-function(df, xi=0, currentDate = Sys.Date()){
    ...
    res <- optim(par=par.inits, fn=DCoptimFn, DCm=dcm, xi=xi, method = "BFGS", hessian=FALSE, currentDate=currentDate)
...
opt_xi <- optimize(min_xi, lower=0, upper=0.05, train=train_2015, schedule=test_schedule,  currentDate=as.Date('2014-12-31'))
```
...
So we see that for the 2015 data only, we get an optimal xi value of `r opt_xi$minimum`, with an RPS of `r opt_xi$objective["rps"]`. We can do optimization for the full data as well. You'll have to believe me when I say that the optimal xi value takes a lot longer to elucidate, but it's `r opt_xi_long$minimum`, giving us an RPS of`r opt_xi_long$objective["rps"]`.
...
```{r random_distributions, echo=FALSE}
library(ggplot2)
library(reshape2)
...
Method|RPS
---|---
Short Training|`r rps_2015$rps`
Long Training|`r rps_all$rps`
Short Training (time weighted)|`r opt_xi$objective["rps"]`
Long Training (time weighted)|`r opt_xi_long$objective["rps"]`
Home Wins Always|`r rps_home$rps`
Even Odds (No OT)|`r rps_even_no_ot$rps`
Even Odds (Plus OT)|`r rps_even$rps`
Randomly Generated Odds|`r rps_rand$rps`
