...
ggplot(elo_all$Meta, 
  ...
  geom_point() +
  ggtitle("Average Elo Ranking by Season") +
  xlab("Season") +
  ylab("Average Elo Ranking")

```

You'll see that every time teams are added, the average ranking goes down, and slowly recovers to 1500 A few times the average goes above the target, this happens when low-ranked teams drop out of the league. By this method, we're currently at `r elo_all$Meta$mean[length(elo_all$Meta$mean)]`, but this will decrease next year as Las Vegas steps into the league.
